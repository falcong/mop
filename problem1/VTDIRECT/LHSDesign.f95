module LHSDesign_MOD
use REAL_PRECISION

contains

SUBROUTINE LATINDESIGN(P, LNSTART, LB, UB, XI, LHSDESIGN)
! LATINDESIGN generates a Latin hypercube experimental design of LNSTART
! points, one of which is XI, in the P-dimensional box
! B = { x | LB <= x <= UB }.  Partition each interval [LB(J),UB(J)] into
! LNSTART equal length subintervals, which partitions the box B into cells.
! A Latin hypercube experimental design has the property that if the
! LNSTART points are projected onto any coordinate direction K, for
! 1 <= K <= P, the projection has exactly one point in each of the
! subintervals of [LB(K),UB(K)].  The cells centered along the diameter
! [LB,UB] of the box B are called the diagonal cells.
!
! The algorithm is to first generate LNSTART points in the diagonal cells,
! stored as the columns of the array LHSDESIGN(1:P,1:LNSTART).  Then
! swap coordinates with the last column so that the given point XI can be
! inserted as the last column.  Finally, permute the coordinates within
! rows of LHSDESIGN(1:P,1:LNSTART-1) by doing LNSTART-2 random interchanges
! in each row.
!
! On input:
!
...
! Local variables.
INTEGER:: I, J, L   ! Temporary indices.
REAL (KIND = R8):: TEMP  ! Used for swapping coordinates.
REAL (KIND = R8), DIMENSION(P):: WORK  ! Work array.
REAL (KIND = R8), DIMENSION(P,LNSTART-2):: SWAP  ! Interchange array.
