...
DO K = 1, N
   ...
   ROBUST: IF ( PRESENT(RLSHEP) ) THEN
      ...
      IF ( MAR > EPSILON( 1.0_R8 ) ) THEN
         FUNC = 1
         ITER = 5
         CALL IRLS(M, N, MAR, ITER, FUNC, OMEGA, BETA, BETA_IN, ALPHA, ALPHA_IN, RES, NP, IER1 )
         IF ( IER1 /= 0 ) THEN
            IER = 3
            CYCLE
         ELSE
...
SUBROUTINE IRLS( M, N,  S, ITER, FUNC, OMEGA, BETA, BETA_IN, ALPHA, ALPHA_IN, RES, NP, IER )
...
! order for the smallest NUM elements. IDIST stores the original label for each 
! element in array DIST. 
! Local variables.
USE REAL_PRECISION
IMPLICIT NONE
