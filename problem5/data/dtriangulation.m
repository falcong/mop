...
if(flg)
	...
	if ~isempty(YY)	
		for i=1:n
			...
			if found
				if YY(found,ndv+M+1)>0.02
					disp('found');
					YY(found,ndv+M+1)=YY(found,ndv+M+1)/2;
					dlta=YY(found,ndv+M+1);
...
%					disp('found elsecase');
				...
				%	disp(norm(XX(argmaxd,ndv+1:ndv+3)-YY(i,ndv+1:ndv+3),2));
					if(norm(XX(argmaxd,1:ndv)-YY(i,1:ndv),2)<0.02 | norm(XX(argmaxd,ndv+1:ndv+M)-YY(i,ndv+1:ndv+M),2)<0.02) 
						iflg=1;
						break;
					end
				end
